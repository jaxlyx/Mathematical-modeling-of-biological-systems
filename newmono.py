...
W = np.zeros((N, N))
for i, j in G.edges():
    W[i, j] = np.random.uniform(0.5, 1.5)  # 权重在 0.8 到 1.2 之间波动
...
# 更新函数
def update(t):
    global V, u, I, average_potentials
    ...
    # 更新输入电流并添加随机噪声
    I = new_I + np.random.uniform(-Noise, Noise, N)
    ...
    # 计算系统平均电位
    average_potential = np.mean(V)
    average_potentials.append(average_potential)
...
# 保存动画为GIF文件
ani.save("izhikevich_network_with_correct_colorbar.gif", writer="pillow")


# 创建一个新的子图，用于绘制平均电位随时间的变化
fig2 = plt.figure(figsize=(10, 4))
ax2 = fig2.add_subplot(111)
ax2.plot(range(T+1), average_potentials, linewidth=1, color='blue')
ax2.set_xlabel('Time Step')
ax2.set_ylabel('Average Membrane Potential (mV)')
ax2.set_title('Average Membrane Potential vs Time')
