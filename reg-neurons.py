...
for i in range(N_subnets):
    subnet_size = np.random.randint(5,6) #调节网络的大小（5-10之间随机）
...
def add_edges_from_subnet_to_main(G, subnet_nodes, main_nodes):
    for subnet in subnet_nodes.values():
        num_edges = 2 #np.random.randint(1, 2)
...
def add_edges_between_subnets(G, subnet_nodes):
    ...
    for i, subnet in subnet_nodes.items():
        ...
        num_edges = 3#np.random.randint(1, 3)
        subnet_nodes_list = list(subnet)
        subnet_node_choices = np.random.choice(subnet_nodes_list, num_edges, replace=False)
...
def update(t):
    ...
    # 计算两个选定的调节网络的平均电位
    avg_potentials = []
    for subnet_index in selected_subnets:
        subnet = subnet_nodes[subnet_index]
        subnet_potentials = [V[i] for i in subnet]
        avg_potential = np.mean(subnet_potentials)
        avg_potentials.append(avg_potential)
        main_potentials = [V[i] for i in range(0,N_main)]
        reg_potentials = [V[i] for i in range(N_main,N_main+15)]
    if len(avg_potentials) == 2:
        x, y = avg_potentials
        avg_potentials_x.append(x)
        avg_potentials_y.append(y)
        ...
        # 连接上一个点和当前点
        if len(avg_potentials_x) > 1:
            ax_phase.plot([avg_potentials_x[-2], avg_potentials_x[-1]], [avg_potentials_y[-2], avg_potentials_y[-1]], color='green')
...
# 示例数据，使用 numpy 数组
plot_multiple_arrays(np.array(range(num)), np.array(avg_potentials_z), np.array(avg_potentials_u), np.array(avg_potentials_x))

def plot_fourier_transform(A, B, C, D):
    """
    此函数将三个数组 B、C、D 相对于数组 A 进行傅里叶变换，并将结果绘制在同一图形中。
    ...
    plt.subplot(3, 1, 1)
    plt.plot(freq, np.abs(fft_B), label='FFT of B')
    plt.title('Fourier Transform of B')
    ...
    plt.subplot(3, 1, 2)
    plt.plot(freq, np.abs(fft_C), label='FFT of C')
    plt.title('Fourier Transform of C')
    plt.xlabel('Frequency')
    plt.ylabel('Magnitude')
    plt.legend()
    plt.grid(True)
    ...
    plt.subplot(3, 1, 3)
    plt.plot(freq, np.abs(fft_D), label='FFT of D')
    plt.title('Fourier Transform of D')
    plt.xlabel('Frequency')
    plt.ylabel('Magnitude')
    plt.legend()
    plt.grid(True)
...
plot_fourier_transform(np.array(range(num)), np.array(avg_potentials_z), np.array(avg_potentials_u), np.array(avg_potentials_x))
